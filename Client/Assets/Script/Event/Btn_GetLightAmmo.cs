...
public class Btn_GetLightAmmo : MonoBehaviour 
{
	...
	IEnumerator FlyToPos()
	{
		...
		while (Vector2.Distance(pSprite.transform.position, P_UI.pthis.ObjAmmoLight.transform.position) > 0.03f)
		{
			yield return new WaitForEndOfFrame();
			ToolKit.MoveTo(gameObject, P_UI.pthis.ObjAmmoLight.transform.position - pSprite.transform.position, 0.8f * fFrame);
			fFrame += 0.05f;
		}
        ...
        SysAchieve.pthis.Add(ENUM_Achievement.Total_LightAmmo, DataPickup.pthis.Data[iItemID].iCount);
		GoogleAnalyticsV3.getInstance().LogEvent("Count", "Pickup LightAmmo", "", 0);
