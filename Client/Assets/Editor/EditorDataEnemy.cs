		...
		foreach(KeyValuePair<GameObject,int> Itor in SysMain.pthis.Enemy)
		{
			if(Itor.Key)
			{
				...
				if(EnemyTemp != null)
				{
					...
					GUILayout.BeginHorizontal("box");
					GUILayout.Label(EnemyTemp.iMonster.ToString(), GUILayout.Width(100.0f));
					GUILayout.Label(EnemyTemp.iHP.ToString(), GUILayout.Width(100.0f));
					GUILayout.Label(Pos.x.ToString(), GUILayout.Width(100.0f));
					GUILayout.Label(Pos.y.ToString(), GUILayout.Width(100.0f));
					GUILayout.EndHorizontal();
				}//if
			}//if
		}//for
